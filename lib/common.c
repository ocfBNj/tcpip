...
ssize_t Recvfrom(int fd, void* buf, size_t nbytes, int flags, struct sockaddr* addr,
                 socklen_t* addr_len) {
    ssize_t n;
...
ssize_t Sendto(int fd, const void* buf, size_t nbytes, int flags, const struct sockaddr* addr,
               socklen_t addr_len) {
    ssize_t n;
...
int udp_client(const char* hostname, const char* service, struct sockaddr** saptr,
               socklen_t* lenp) {
    int n;
    int fd;
...
void connect_information(const struct sockaddr* addr) {
    ...
    if (addr->sa_family == AF_INET) {
        ...
        fprintf(stdout, "connected by %s, port %d\n",
                inet_ntop(AF_INET, &ipv4->sin_addr, ip, sizeof ip), ntohs(ipv4->sin_port));
    } else if (addr->sa_family == AF_INET6) {
        struct sockaddr_in6* ipv6 = (struct sockaddr_in6*)addr;
        fprintf(stdout, "connected by %s, port %d\n",
                inet_ntop(AF_INET6, &ipv6->sin6_addr, ip, sizeof ip), ntohs(ipv6->sin6_port));
    } else {
